...
describe('PATCH /transactions/:id/reject', () => {
  ...
  it('Debe rechazar una transacción PENDING y descongelar los fondos de la cuenta origen', async () => {
    const res = await request(ctx.app.getHttpServer())
      .patch(`/transactions/${pendingTransaction.id}/reject`)
      .set('Authorization', `Bearer ${ctx.authToken}`);

    expect(res.status).toBe(ResponseStatusCode.OK);
    expect(res.body.data.status).toBe(TransactionStatus.REJECTED);

    // Verificar que el saldo fue descongelado
    ...
    // Verificar que el destino NO recibió nada
  ...
  it('Debe retornar error si se intenta rechazar una transacción que ya fue APPROVED', async () => {
    ...
    expect(res.status).toBeGreaterThanOrEqual(ResponseStatusCode.BAD_REQUEST);
    expect(res.body.success).toBe(false);
  });

  it('Debe retornar error si se intenta rechazar una transacción que ya fue REJECTED', async () => {
    await request(ctx.app.getHttpServer())
      .patch(`/transactions/${pendingTransaction.id}/reject`)
      .set('Authorization', `Bearer ${ctx.authToken}`)
      .expect(ResponseStatusCode.OK);
